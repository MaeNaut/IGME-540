...
Sky::Sky(std::shared_ptr<Mesh> _mesh, Microsoft::WRL::ComPtr<ID3D11SamplerState> _sampler, std::wstring filePath)
{
	mesh = _mesh;
	sampler = _sampler;

	skyboxTexture = CreateCubemap(
...
void Sky::Draw(std::shared_ptr<Camera> camera)
{
	...
	ps->SetShaderResourceView("SkyboxTexture", skyboxTexture);
